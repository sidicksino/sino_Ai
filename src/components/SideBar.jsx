...
const SideBar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  return (
    ...
    transition-all duration-500 left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}> 
      {/* Logo */}
      <img src={theme === 'dark' ? assets.sino_logo_ai_dark : assets.sino_logo_ai} alt="" className='w-full max-w-48'/>
      ...
      <div onClick={()=> {navigate('/credits'); setIsMenuOpen(false)}} className='flex items-center gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
        ...
        <div className='flex flex-col text-sm'>
            ...
            <p className='text-sm text-gray-400'>Purchase credits to use SINO Ai</p>
        </div>
      </div>
      ...
      <img src={assets.close_icon} onClick={()=> setIsMenuOpen(false)} className='absolute top-3 right-3 w-5 h-5 cursor-pointer md:hidden not-dark:invert' alt="" />
    </div>
  )
}
