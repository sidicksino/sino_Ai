...
const ChatBox = () => {
  ...
  const onSubmit = async (e) => {
    ...
    // if(!prompt) return
    // setLoading(true)
    // setMessages([...messages, { role: 'user', content: prompt, timestamp: new Date().toISOString() }])
    
    // // Simulate an AI response
    // setTimeout(() => {
    //   setMessages(prev => [...prev, { role: 'assistant', content: `AI response to: ${prompt}`, timestamp: new Date().toISOString() }])
    //   setLoading(false)
    //   setPrompt('')
    // }, 1000)
  }
